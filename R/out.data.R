...
#out.data
out.data<-function(input,vpd.input=vpd,sr.input=sr,prec.input=prec,peak.hours=c(10:14),low.sr = 0.15,
         peak.sr=0.75,vpd.cutoff= 0.5,prec.lim=1,method="env.filt",make.plot=TRUE){
...
if(is.numeric(low.sr)==F)stop("Unused argument, low.sr need to be numeric.")
if(is.numeric(peak.sr)==F)stop("Unused argument, peak.sr need to be numeric.")
if(is.numeric(vpd.cutoff)==F)stop("Unused argument, vpd.cutoff need to be numeric.")
if(is.numeric(prec.lim)==F)stop("Unused argument, prec.lim need to be numeric.")
...
if(method == 'env_filt'){
  ...
  add2<-data.frame(sr=as.numeric(sr_df_daily$x),doy_y=as.numeric(sr_df_daily$doy_y))
  adding<-merge(add,add2,by="doy_y")
  ...
  sfd_df[which(sfd_df$sr_day<as.numeric(sr_limit)),"vpd_filt"]<-NA
  sfd_df[which(sfd_df$sr.input<quantile(sfd_df$sr.input, probs = peak.sr,na.rm=T)),"gc_filt"]<-NA
  sfd_df[which(sfd_df$sr.input<quantile(sfd_df$sr.input, probs = peak.sr,na.rm=T)),"vpd_filt"]<-NA
  ...
  sfd_df_peak_daily<-sfd_df_peak_daily[which(sfd_df_peak_daily$gc_fil!="NaN"|sfd_df_peak_daily$vpd_fil!="NaN"),]
  ...
  sfd_df_4gcfit$gc_norm = as.numeric(sfd_df_4gcfit$gc_filt)/max(sfd_df_4gcfit$gc_filt, na.rm=T)
...
par(oma=c(1,1,1,1))
par(mfrow=c(1,1))
plot(df$x,df$y,pch=16,col="white",cex=2.5,yaxt="n",ylab="",xlab="",ylim=c(0,max(df$y)))
axis(side=2,las=2)
points(df$x,df$y,col="grey",pch=16,cex=1.8)
points(df$x,df$y,col="white",pch=1,cex=1.8)
points(df$x,df$y,col="grey",pch=16,cex=1.6)
...
legend("top",c(expression(italic("G")[c]*" = "*italic(alpha)*" + "*italic(beta)*" VPD"^0.5),
...
mtext(side=2,expression(italic(G)[c]*" ("*"mmol "*m^-2*" "*s^-1*" "*kPa^-1*")"),padj=-2.5)
mtext(side=1,expression(VPD*" (kPa)"),padj=3)
}

#o= output
raw<-zoo::fortify.zoo(sfd_df)
colnames(raw)<-c("timestamp","sfd","vpd","sr","doy","hour","year","doy.y","gc","sr.day","prec.day","gc.filt","vpd.filt")
raw<-raw[,c(1,7,5,6,4,10,3,13,11,2,9,12)]
peak.mean<-sfd_df_4gcfit
colnames(peak.mean)<-c("timestamp","sfd","vpd","sr","doy","hour","year","doy.y","gc","sr.day","prec.day","gc.filt","vpd.filt","gc.norm")
...
output<-out.data(input,vpd.input=vpd.input,sr.input=sr.input,prec.input=prec.input,prec.lim=1,method="env.filt",make.plot=TRUE)
head(output)
